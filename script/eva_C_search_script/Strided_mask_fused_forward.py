...
@torch.jit.script
def fused_with_jit_bias0_QK_mask(qkv_bias_this, qkv, mask):
...
@torch.jit.script
def fused_with_jit_sfmx_pV(v, scores):
...
@torch.jit.script
def fused_with_jit_gemm1_bias1(attr_output_kernel_this, attr_output_bias_this, input_tensor, hidden_states):
...
@torch.jit.script
def fused_with_jit_bias2act_gemm3_bias3(inter_bias_this, output_kernel_this, output_bias_this, residual, hidden_states):
    ...
    for i in range(warmup_iters + iters):
        ...
        with torch.no_grad():
            ...
            for layer in range(layer_num):
                ...
                v, scores = fused_with_jit_bias0_QK_mask(qkv_bias_this, qkv, mask)

                h = fused_with_jit_sfmx_pV(v, scores)
                ...
                hidden_states = fused_with_jit_gemm1_bias1(attr_output_kernel_this, attr_output_bias_this, input_tensor, hidden_states)
                ...
                hidden_states = fused_with_jit_bias2act_gemm3_bias3(inter_bias_this, output_kernel_this, output_bias_this, residual, hidden_states)
