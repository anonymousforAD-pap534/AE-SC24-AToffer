...
@torch.jit.script
def fused_with_jit_bias1_lynorm1(attr_output_bias_this, attr_output_layernorm_gamma_this, attr_output_layernorm_beta_this, input_tensor, hidden_states):
...
@torch.jit.script
def fused_with_jit_gemm2_bias2act_gemm3_bias3_lynorm3(inter_kernel_this, inter_bias_this, output_kernel_this, output_bias_this, output_layernorm_gamma_this, output_layernorm_beta_this, residual, hidden_states):
    ...
    for i in range(warmup_iters + iters):
        ...
        with torch.no_grad():
            ...
            for layer in range(layer_num):
                ...
                hidden_states = syncfree_attention.run_syncfree_fixed_attention(q, k, v)

                hidden_states = torch.matmul(hidden_states, attr_output_kernel_this)      # 7 GEMM1
                
                residual, hidden_states = fused_with_jit_bias1_lynorm1(attr_output_bias_this, attr_output_layernorm_gamma_this, attr_output_layernorm_beta_this, input_tensor, hidden_states)

                hidden_states = fused_with_jit_gemm2_bias2act_gemm3_bias3_lynorm3(inter_kernel_this, inter_bias_this, output_kernel_this, output_bias_this, output_layernorm_gamma_this, output_layernorm_beta_this, residual, hidden_states)
