...
@torch.jit.script
def fused_with_jit_gemm0_bias0_QK_mask_sfmx(qkv_kernel_this, qkv_bias_this, mask, hidden_states):
	...
	scores = torch.matmul(q, k.transpose(-2, -1)) / (64** .5)                   # 2 GEMM Q*K Attention start
	
	scores -= 10000.0 * (1.0 - mask.unsqueeze(1))                               # 3 mask
	
	probs = F.softmax(scores, dim=-1)                                           # 4 softmax
	
	return input_tensor, v, probs


@torch.jit.script
def fused_with_jit_trans_gemm1(attr_output_kernel_this, h, hidden_states):
	h = h.permute(0, 2, 1, 3).contiguous()                                      # 6 transpose
	new_context_layer_shape = h.size()[:-2] + (768, )
...
@torch.jit.script
def fused_with_jit_bias1_lynorm1_gemm2_bias2act_gemm3_bias3_lynorm3(attr_output_bias_this, attr_output_layernorm_gamma_this, attr_output_layernorm_beta_this, inter_kernel_this, inter_bias_this, output_kernel_this, output_bias_this, output_layernorm_gamma_this, output_layernorm_beta_this, input_tensor, hidden_states):
    ...
    for i in range(warmup_iters + iters):
        ...
        with torch.no_grad():
            ...
            for layer in range(layer_num):
                ...
                input_tensor, v, probs = fused_with_jit_gemm0_bias0_QK_mask_sfmx(qkv_kernel_this, qkv_bias_this, mask, hidden_states)

                h = torch.matmul(probs, v)                                                  # 5 GEMM QK*V
                
                hidden_states = fused_with_jit_trans_gemm1(attr_output_kernel_this, h, hidden_states)

                hidden_states = fused_with_jit_bias1_lynorm1_gemm2_bias2act_gemm3_bias3_lynorm3(attr_output_bias_this, attr_output_layernorm_gamma_this, attr_output_layernorm_beta_this, inter_kernel_this, inter_bias_this, output_kernel_this, output_bias_this, output_layernorm_gamma_this, output_layernorm_beta_this, input_tensor, hidden_states)
